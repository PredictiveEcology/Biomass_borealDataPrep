...
modversion <- paste(unlist(SpaDES.core::moduleMetadata(module = 'Biomass_borealDataPrep', path = '..')$version), collapse = ".")
download.file(url = paste0("https://img.shields.io/badge/Biomass_borealDataPrep-", paste("v.", modversion),
                           "-%3CCOLOR%3E.png"),
              destfile = "figures/moduleVersionBadge.png",
              mode = 'wb')
```

[![module-version-Badge](figures/moduleVersionBadge.png)](https://github.com/CeresBarros/Biomass_borealDataPrep/commit/665d6db36f5f12d3c5068aa3e5ecc874e84f51f0)
...
knitr::include_graphics(normalizePath(c("figures/biomassModel_logBtest.png"), winslash = "/"))
```

```{r fig-simBLogBtest, eval=TRUE, echo=FALSE, fig.align = 'center', out.width = "70%", fig.cap = "Thirty years of simulation with `maxB` values estimated from a `logB ~ ...` `biomassModel` (see Fig. \\@ref(fig:fig-biomassModelLogBtest)). The steep increase in such little time is abnormal."}
## make sure all include_graphics have wd attached to figure path
## this is necessary to knit to pdf using child Rmds - see https://stackoverflow.com/questions/61065560/cant-compile-rmarkdown-pdf-with-image-in-child-rmd-latex-error-image-file
knitr::include_graphics(normalizePath(c("figures/simulatedB_logBtest.png"), winslash = "/"))
```
...
```{r mergingSpp-Biomass-borealDataPrep, echo = FALSE, eval = TRUE, message = FALSE}
