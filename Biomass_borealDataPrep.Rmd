...
-   Module initiation (`init` event): after downloading all the necessary data
    ...
    objects and parameters for the simulation (see 'Data dependencies' below).
    Depending on the objects, some are parameterized using empirical models,
    others based on literature (e.g., longevity values for western boreal
    species taken from Burton & Cumming (1995) -- see
    `?LandR::speciesTableUpdate`), or expert knowledge (e.g., `sufficientLight`
    values adjusted to reflect western boreal forest succession dynamics).
    prepares all inputs and parameters necessary to run *Biomass_core*
...
imputing missing data in some cases, which are presented thoroughly in
<!-- section -->.
...
1.  **invariant species traits** -- spatio-temporally constant traits that
    ...
    and responses to fire, and including include the **probabilities of
    germination** for a given species tolerance and site shade combination
    (`suffiencientLight` table) which link species shade tolerance values
    (`shadetolerance`) with site shade (determined by `minRelativeB`) to
    simulate germination success in any given pixel;
...
within forested land-cover classes (see <!-- section -->).
...
tolerance and site shade and biomass thrsholds defining site shade levels
(`minRelativeB`) were obtained from a combination of published literature (e.g.,
longevity values followed Burton & Cumming, 1995) and values used in LANDIS-II
applications in Canada's boreal forests. Default `minRelativeB` values are kept
constant across all ecolocations due to the lack of data needed to derive
ecolocation-specific values (see <!-- section -->). They are also adjusted by
lowering the values of higher shade classes to reflect lower shade levels
observed in Western Canadian forests with respect to their Eastern counterparts
at similar density levels (Messier et al., 1998), which are likely driven by
higher moisture limitation in the west (Hogg et al., 2008; Peng et al., 2011).
...
is `LandR::makeMinRelativeB`; see <!-- section --> for further detail).
...
Beaudoin et al., 2017) -- hereafter 'kNN species data'.
...
by the NFI (Beaudoin et al., 2017), by default. *Biomass_borealDataPrep*
downloads these data and performs a number of data harmonization operations to
...
relationship of conifer and broadleaf species (see <!-- section -->).
...
Canadian Ecozone (NRCan, 2013), which are by default filtered to the Boreal
...
Longevity values are adjusted to match the values from Burton and Cumming
(1995), which match BSP, BP and MC ecozones. These adjustments result in higher
longevity for most species; - Shade tolerance values are lowered for *Abies
balsamifera*, *Abies lasiocarpa*, *Picea engelmanii*, *Picea glauca*, *Picea
mariana*, *Tsuga heterophylla* and *Tsuga mertensiana* to better **relative**
shade tolerance levels in Western Canada. Because these are realtive shade
tolerances, the user should **always** check these values with respect to their
own study areas and species pool.
...
forests with regards to Eastern Canadian forests (Messier et al., 1998).
...
are estimated statistically (see <!-- section -->)
...
-   **Tables**
    ...
    -   `speciesTable` -- a table of invariant species traits that must have the
        ...
        Scheller and Mladenof (2015) for details about these columns.
