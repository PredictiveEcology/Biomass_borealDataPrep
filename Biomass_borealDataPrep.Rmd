...
Require(c("SpaDES.core", "git2r", "openxlsx"), require = FALSE, install = FALSE)
Require(c("dplyr", "data.table", "kableExtra",
...
badgeURL <- if (!git2r::is_detached(modulePath)) {
  commitSHA <- git2r::sha(git2r::revparse_single(modulePath, "HEAD"))
  repo <- sub("[.]git$", "/commit/",
              git2r::branch_remote_url(git2r::branch_get_upstream(git2r::repository_head(modulePath))))
  paste0(repo, commitSHA)
} else {
  ## if detached point to the first remote
  git2r::remote_url(modulePath)[1]
}
...
# knitr::kable(sppEquiv2, caption = caption) %>%
#   kableExtra::kable_styling(latex_options = c("repeat_header"), full_width = TRUE) %>%
#   kableExtra::column_spec(column = which(names(sppEquiv2) %in% c("Species", "Modelled as")),
#                           italic = TRUE)

justCols <- sapply(sppEquiv2, function(x) ifelse(is.numeric(x), "right", "left"))
# pander(sppEquiv2, caption = addLabel(caption), justify = justCols,
...
# options(opts)
panble(df_inputs, landscape = TRUE, caption, panderArgs = list("justify" = "left", "split.tables" = Inf,
...
# options(opts)
panble(df_params, caption, landscape = TRUE, panderArgs = list("justify" = "left", "digits" = 3, "split.cells" = c(15,15, 5, 5, 5, 40), "split.tables" = Inf))
```
...
# knitr::kable(df_outputs, caption = caption) %>%
#   kableExtra::kable_styling(latex_options = c("repeat_header")) %>%
