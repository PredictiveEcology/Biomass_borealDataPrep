...
Require(c("SpaDES.core", "git2r", "kableExtra",
          "dplyr", "data.table", "PredictiveEcology/LandR"), install = TRUE)
```
...
commitSHA <- git2r::sha(git2r::revparse_single(".", "HEAD"))
  repo <- sub(".*github.com:", "https://github.com/",
      git2r::branch_remote_url(git2r::branch_get_upstream(git2r::repository_head("."))))
  repo <- sub(".git$", "/commit/", repo)
  paste0(repo, commitSHA)
} else {
  ## if detached point to the first remote
  badgeURL <- git2r::remote_url()[1]
}

badgePath <- normalizePath("figures/moduleVersionBadge.png", winslash = "/")

## make string of markdown code to be executed as-is
paste0("[![module-version-Badge](", badgePath, ")](", badgeURL,")")
```
...
badgePath <- normalizePath("figures/issuesBadge.png", winslash = "/")

## make string of markdown code to be executed as-is
paste0("[![Issues-badge](", badgePath, ")](", badgeURL,")")
```
...
LandR *Biomass_borealDataPrep* (hereafter *Biomass_borealDataPrep*), 
prepares all necessary inputs for *Biomass_core* based on data
available for forests across Canada forests, but focused on Western Canada
boreal forest systems. Nevertheless, it provides a good foundation to develop
other other modules aimed at different geographical contexts. By keeping data
preparation and parameter estimation outside of *Biomass_core*, we promote the
modularity of the LandR-based model systems and facilitate interoperability with
other parameter estimation procedures.
...
*Biomass_borealDataPrep* requires internet access to retrieve default data[^biomass_borealdataprep-1].
...
repeated tree biomass measurements) across
...
[^biomass_borealdataprep-1]: Raw data layers downloaded by the module are saved in \`dataPath(sim)\`, which can be controlled via \`options(reproducible.destinationPath = ...)\`.
